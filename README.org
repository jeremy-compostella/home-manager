...
#+begin_src python :session :exports none :results output :dir ./src
    ...
    def daily_data():
        ...
        with get_database() as database:
            ...
            req += ' WHERE timestamp>=\'2022-01-01\' AND timestamp<\'2023-01-01\''
            cursor.execute(req)
            daily_energy = cursor.fetchall()

        for day in daily_energy:
            day['other'] = -(total(day) + day['solar'] - day['net'])
            for key, value in day.items():
                if key != 'timestamp':
                    day[key] = abs(value)
            day['hvac'] = day['a_c'] + day['air_handler']
            day['local'] = day['solar'] - day['to_grid']
            del day['a_c']
            del day['air_handler']
            day['timestamp'] = parser.parse(day['timestamp'])
...
As detailed in section [[Financial and environmental results]], the use of solar panels, electric cars and software optimizations allow us to save at the very minimum *USD 2,500 per year* and to reduce our carbon emissions by more than *7.1 tons of CO_{2} per year*.
...
From call_db-first-month() {{{results(=September 2021=)}}} to call_db-last-month() {{{results(=August 2022=)}}}, the entire home energy consumption has been covered by solar energy at call_powered-by-solar() {{{results(=77%=)}}}.
...
#+name: energy_consumption_distribution
...
#+begin_src python :session: :results file :exports results :dir ./src
  ...
  END_TIME_STR=DATE_STR + ' 16:30:00'
...
#+begin_src python :session :exports none :results value :var data=monthly-data(12) :dir ./src
  ...
  '%.1f' % (((used * .294) - (used * 0.041)) / 1000)
#+end_src
[[https://app.electricitymaps.com/zone/US-SW-SRP][Assuming an average of 294 gCO_{2}eq/kWh for the electricity produced by our power utility on twelve months]] compared to the [[https://www.ipcc.ch/site/assets/uploads/2018/02/ipcc_wg3_ar5_annex-iii.pdf][41 gCO_{2}eq/kWh]] (Table A.III.2 | Emissions of selected electricity supply technologies) from our solar panels, in the last twelve month, we have reduced our carbon emission by *call_home-usage() {{{results(=1.7=)}}} tons*.
...
#+begin_src python :session :exports none :results value :dir ./src :var bolt=bolt-distance :var model3=model3-distance
  '{:,}'.format(int((eval(bolt.replace(',', '')) + eval(model3.replace(',', ''))) / 33 * 3.6))
#+end_src
...
#+begin_src python :session :exports none :results value :dir ./src :var kwh=ev-charging-kwh :var bolt=bolt-distance :var model3=model3-distance
  '%.1f' % ((((int(bolt.replace(',', '')) + int(model3.replace(',', ''))) / 30 * 8.887 * 1.3) - int(kwh.replace(',', '')) * 0.041) / 1000)
#+end_src

#+name: ev-savings-co2-per-year
#+begin_src python :session :exports none :results value :dir ./src :var data=ev-savings-co2
  '%.1f' % (data / (datetime.now().year + (datetime.now().day / 365) - 2021.328))
#+end_src
...
#+RESULTS: model3-distance
: 6,198

Since May 2021, we have been driving call_bolt-distance() miles with our Bolt EV and since May 2022 call_model3-distance() {{{results(=9\,349=)}}} miles with our Model 3. I am purposefully excluding driving not powered by electricity from our home. We used a total of call_ev-charging-kwh()  KWh from our solar panels to charge the cars. Considering that we used this electricity instead of selling it back to our power utility it "cost" us *USD call_ev-charging-usd() {{{results(=249=)}}}*.

If we assume we were driving a gas car with a mileage of 30 miles/gallon and an average gas price of 3.60 USD/gallon it would have cost us USD call_gas-equivalent-usd() . It means that we saved *USD call_ev-savings-usd() {{{results(=2\,223=)}}}* on gas.

More importantly, assuming that a gallon of gas produces [[https://www.epa.gov/greenvehicles/greenhouse-gas-emissions-typical-passenger-vehicle][8.887 kg of CO_{2}]] plus [[https://innovationorigins.com/en/producing-gasoline-and-diesel-emits-more-co2-than-we-thought/][30% since gas does not magically materializes at the gas station]]. Including construction, installation and recycling, one KWh of electricity produced by our solar panels system creates 41g of CO_{2}. Therefore we have avoided the emission of *call_ev-savings-co2() {{{results(=14.4=)}}} tons of CO_{2}*, about *call_ev-savings-co2-per-year()* *tons CO_{2} / year*.

However, we have to take into account the production of the battery of both cars as they can be considered extra pollution compared to a gas car. If we assume the absolute nearly impossible worst  case scenario where it would not get recycled at all and we consider that creating a [[https://changeit.app/blog/2021-03-26-environmental-impact-of-lithium-batteries/][1 KWh battery generates 73 Kg of CO_{2}]], our two EV batteries created around 10.3 tons of CO_{2}. Therefor, at our current rate, it took less than a year and a half to break even with two gas cars.

* What would be the impact of adding a battery ?
#+name: max-from-grid
#+begin_src python :session :exports none :results value :dir ./src :var data=daily-data
  import datetime
  data = eval(data)
  '%.1f' % (max([d['from_grid'] for d in data]))
#+end_src

#+name: max-battery
#+begin_src python :session :exports none :results value :dir ./src :var data=max-from-grid
  '%.1f' % (data * 73)
#+end_src

In the elapsed year, the maximum quantity of energy we pulled from the grid in one day is call_max-from-grid() {{{results(=38.3=)}}} KWh. Therefore, if we want to cover all our needs and assuming it is sunny every single day, we would need to at least double our solar installation and add a call_max-from-grid() {{{results(=38.3=)}}} KWh battery. It seems a bit like a huge investment which may not yield the best environmental results.
...
- [[https://changeit.app/blog/2021-03-26-environmental-impact-of-lithium-batteries/][1 KWh battery generates 73 Kg of CO_{2}]]
- My utility company generates 294 of CO_{2} / KWh and my solar panel installation is at  [[https://www.ipcc.ch/site/assets/uploads/2018/02/ipcc_wg3_ar5_annex-iii.pdf][41 gCO_{2}eq/kWh]] .
...
#+begin_src python :session: :results file :exports results :var data=daily-data :var extra_panels_ratio=0 :eval no-export :var max_battery=5 :var step=100
  import matplotlib.pyplot as plt
  import datetime

  data = eval(data)

  def saved(data, battery_size):
      level = 0
      total = 0
      for d in data:
          from_grid = max(0, d['from_grid'] - level)
          level -= d['from_grid']
          level += d['to_grid'] + extra_panels_ratio * d['solar']
          if level < 0:
              level = 0
          if level > battery_size:
              level = battery_size
          total += d['from_grid'] - from_grid
      return total

  res = []
  start = int(0.1 * step)
  for i in range(start, max_battery * step):
      res.append((i / step * 73) / (saved(data, i / step) * 0.253))
  minimal = {'battery_size':0, 'years':100}
  for i in range(0, max_battery * step - start):
      if res[i - 1] < minimal['years']:
          minimal['battery_size'] = (i + start) / step
          minimal['years'] = res[i - 1]
  plt.xlabel('Battery size (KWh)')
  plt.plot([(x + start) / step for x in range(0, max_battery * step - start)], 
           res, label='Years')
  plt.grid(which='major', linestyle='dotted')
  plt.scatter(minimal['battery_size'], minimal['years'], s=80, label=f'Optimal battery size: {minimal["battery_size"]} KWh')
  plt.gca().set_title('Number of years to offset the battery CO2eq cost')
  plt.legend(loc='best')
  fname = f'../doc/images/simulation-battery-years-to-offset.svg'
  plt.tight_layout()
  plt.savefig(fname)
  return fname
#+end_src

[[file:doc/images/simulation-battery-years-to-offset.svg]]

\rArr  the optimal battery size is about 3 KWh. It would save about 220 CO_{2} per year which will be offset in about two years.
