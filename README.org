...
From call_db-first-month() {{{results(=September 2021=)}}} to call_db-last-month() {{{results(=August 2022=)}}}, the entire home energy consumption has been covered by solar energy at call_powered-by-solar() {{{results(=73%=)}}}.
...
Since May 2021, we have been driving call_bolt-distance() miles with our Bolt EV and since May 2022 call_model3-distance() miles with our Model 3. I am purposefully excluding driving not powered by electricity from our home. We used a total of call_ev-charging-kwh()  KWh from our solar panels to charge the cars. Considering that we used this electricity instead of selling it back to our power utility it "cost" us *USD call_ev-charging-usd() {{{results(=178=)}}}*.

If we assume we were driving a gas car with a mileage of 30 miles/gallon and an average gas price of 3.40 USD/gallon it would have cost us USD call_gas-equivalent-usd() . It means that we saved *USD call_ev-savings-usd() {{{results(=2\,223=)}}}* on gas.
